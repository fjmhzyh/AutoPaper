    ...
    *,
    ...
    if host != "www.nature.com":
        return {
            ...
            "paper_file": "",
            "si_file": "",
            "failed_reason": f"nature域名未支持: {host or 'unknown'}",
        }

    paper_url = _extract_nature_paper_url(html_content, resolved_url)
    if not paper_url:
        logger.warning("[论文下载] nature未找到论文下载链接")
        return {
            "paper_ok": False,
            "si_ok": False,
            "paper_download_url": "",
    ...
    si_url = _extract_nature_si_url(html_content, resolved_url)
    if not si_url:
        logger.info("[资料下载] nature未找到si下载链接")
        si_ok = False
        si_file = ""
    else:
...
def _extract_nature_paper_url(html_content: str, base_url: str) -> str:
    text = str(html_content or "")
    if not text:
        return ""

    match = re.search(
        r"""<a[^>]*href=["']([^"']+)["'][^>]*data-article-pdf=["']true["'][^>]*>""",
        text,
        flags=re.IGNORECASE,
    )
    if not match:
...
def _extract_nature_si_url(html_content: str, base_url: str) -> str:
    text = str(html_content or "")
    if not text:
        return ""

    match = re.search(
        r"""<a[^>]*class=["'][^"']*print-link[^"']*["'][^>]*href=["']([^"']+)["'][^>]*>""",
        text,
        flags=re.IGNORECASE,
    )
    if not match:
