...
PAPER_TEMPLATE = "https://www.tandfonline.com/doi/pdf/{doi}"
    ...
    *,
    ...
    if host != "www.tandfonline.com":
        return {
            ...
            "paper_file": "",
            "si_file": "",
            "failed_reason": f"tandfonline域名未支持: {host or 'unknown'}",
        }
    ...
    time.sleep(20)
    content_unavailable, = utils.check_keywords_exist(["content unavailable"])
    logger.info(f"下载地址:{paper_download_url}")
    if content_unavailable:
        logger.warning("[论文下载] tandfonline下载失败 - content unavailable")
        return {
            "paper_ok": False,
            "si_ok": False,
            ...
            "paper_file": "",
            "si_file": "",
            "failed_reason": "content unavailable",
        }
    else:
...
def download_si(
    ...
    *,
    ...
    si_url = ""
    for keyword in ["supplement", "supplemental", "supplementary", "supporting"]:
        si_url = find_link_by_keyword(links, keyword, resolved_url)
        if si_url:
            break
    if not si_url:
        logger.info("[资料下载] tandfonline未找到si下载链接")
        return False, ""
    ...
    if not si_file:
        logger.warning("[资料下载] tandfonline si下载失败 - 打印保存未生成文件")
        return False, ""
